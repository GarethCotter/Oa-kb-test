...
| `api/suggest.js` | Validates, then forwards to the Apps Script webhook. |
...
Eleven columns: Timestamp, Name, Email, Type, Suggestion, Product area, Article or
page, Link, How often, **Status**, **Notes**.
...
Four required — type, the suggestion, name, email. Everything else is optional and
labelled as such, because a form that feels like paperwork gets one submission a month.
...
- "Send another" → keeps name and email, clears the rest
- 375px wide → no horizontal overflow; brand values exact (cream `#EDEBE2`, navy
  `#101C38`, red `#D0432C`, Gloock + Outfit)
